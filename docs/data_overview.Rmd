...
bucketPlot <- function(bt,x,y){
  ...
  ggplot(foo, aes(variable, value)) +  geom_bar(stat = "identity") + xlab(x)+ylab(y) + theme(axis.text.x = element_text(angle = 90, hjust = 1))
}
```
...
Pensja<-ggplot(dataset, aes(x=V8)) +  geom_bar() + xlab("Pensja") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) + ylab("Liczba osób") + ggtitle("Pensja programisty netto/msc")
