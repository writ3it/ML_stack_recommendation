...
bucketPlot <- function(bt){
  foo <- bt %>% summarise_each(funs(sum(.==1)))
  foo <- melt(foo) %>% transform(variable=reorder(variable, -value))
  ggplot(foo, aes(variable, value)) +  geom_bar(stat = "identity") + xlab("Testy") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
}
```
## Technologie
``` {r V28}
...
bucketPlot(bt)
```
## Histogramy Wiek, Pensja, Stanowisko, Wojewódtzwo
```{r V5}
...
Wojewodtzwo<-ggplot(dataset, aes(x=V4)) +  geom_bar() + xlab("Wojewodtzwo") + theme(axis.text.x = element_text(angle = 90, hjust = 1))
